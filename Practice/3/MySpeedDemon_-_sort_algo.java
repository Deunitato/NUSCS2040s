...
public class MySpeedDemon {
	...
	public static String sortString(String s){
		ArrayList<Character> myfakelist = new ArrayList<>();
		for(int i = 0; i < s.length(); i++){
			char c = s.charAt(i); 
			myfakelist.add(c);
		}
        Collections.sort(myfakelist); 

		// return new sorted string 
	   StringBuilder builder = new StringBuilder(myfakelist.size());
       for(Character ch: myfakelist)
       {
        builder.append(ch);
       }
